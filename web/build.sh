...
if ! command -v "$WB" >/dev/null 2>&1 && [[ ! -x "$WB" ]]; then
  ...
  echo "       Install with: cargo install wasm-bindgen-cli --version $(awk '/^name = "wasm-bindgen"$/{getline; gsub(/[",]/,""); print $3; exit}' Cargo.lock)" >&2
...
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> wasm-opt -Oz"
  BEFORE=$(du -h "$OUT/metalworld_bg.wasm" | awk '{print $1}')
  wasm-opt -Oz --enable-bulk-memory --enable-nontrapping-float-to-int \
    -o "$OUT/metalworld_bg.wasm.opt" "$OUT/metalworld_bg.wasm"
  mv "$OUT/metalworld_bg.wasm.opt" "$OUT/metalworld_bg.wasm"
  echo "    ${BEFORE} -> $(du -h "$OUT/metalworld_bg.wasm" | awk '{print $1}')"
else
...
echo "Built ${OUT}:"
du -sh "$OUT"
du -h "$OUT/metalworld_bg.wasm" | awk '{print "  wasm: " $1}'
echo
