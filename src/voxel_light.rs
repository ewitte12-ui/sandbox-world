...
use crate::chunk::{CHUNK_SIZE, CHUNK_VOLUME, Chunk, ChunkNeighbors, ChunkPos};
...
pub fn predicted_sky(wy: i32, surface: i32) -> u8 {
    if wy > surface { MAX_LIGHT } else { 0 }
}
...
pub fn compute_light(blocks: &[BlockType; CHUNK_VOLUME], shell: &ShellLight) -> [u8; CHUNK_VOLUME] {
...
pub fn boundary_faces_changed(old: &[u8; CHUNK_VOLUME], new: &[u8; CHUNK_VOLUME]) -> [bool; 6] {
        ...
        let mut relax = |a: &mut Chunk, b: &mut Chunk| -> u32 {
            for round in 0..40 {
                let na = compute_chunk_light(
                    ...
                    a.pos,
                    &ChunkNeighbors {
                        neighbors: [None, Some(b), None, None, None, None],
                    },
                );
                ...
                let nb = compute_chunk_light(
                    ...
                    b.pos,
                    &ChunkNeighbors {
                        neighbors: [Some(a), None, None, None, None, None],
                    },
                );
