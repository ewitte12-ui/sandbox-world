...
// The custom-block feature is wired into chunk_manager's registry-invalidation
// path, but nothing creates custom blocks until there is a texture menu, so its
// surface reads as dead today.
#[allow(dead_code)]
...
impl CustomBlockRegistry {
    ...
    pub fn next_index(&self) -> Option<u8> {
        ...
        (CUSTOM_BLOCK_START..MAX_BLOCK_TYPES)
            .find(|idx| !self.blocks.iter().any(|b| b.atlas_index == *idx))
    }
...
impl BlockType {
    ...
    pub fn color(&self) -> Color {
        if let Ok(guard) = CUSTOM_COLORS.read()
            && let Some(ref colors) = *guard
            && let Some(rgba) = colors.get(self.name())
        {
    ...
    #[allow(dead_code)] // see the custom-block note above
