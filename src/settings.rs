...
/// Persistent game settings, saved to ~/.sandbox_world_settings.json
...
fn default_ssao() -> bool {
    false
}
fn default_ssao_quality() -> String {
    "medium".to_string()
}
fn default_smaa() -> String {
    "off".to_string()
}
...
fn default_clouds() -> bool {
    true
}
fn default_texture_size() -> u32 {
    64
}
fn default_aniso() -> u16 {
    1
}
...
impl GameSettings {
    ...
    fn settings_path() -> std::path::PathBuf {
        crate::platform::home_dir().join(".sandbox_world_settings.json")
    }
}
...
impl Plugin for SettingsPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(GameSettings::load()).add_systems(
            Update,
            sanitize_settings.run_if(resource_changed::<GameSettings>),
        );
    }
}
