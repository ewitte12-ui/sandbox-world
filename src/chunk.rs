...
use block_mesh::{
    RIGHT_HANDED_Y_UP_CONFIG, UnitQuadBuffer,
    ndshape::{ConstShape, ConstShape3u32},
    visible_block_faces,
};

use std::collections::HashMap;
// Only the debug-only greedy-merge verification (and the tests) need this, so
// a release build has no use for the import.
...
impl Chunk {
    ...
    pub fn generate_tracked(pos: ChunkPos) -> (Self, [bool; 6]) {
        ...
        let boundary_mask: [bool; 6] = std::array::from_fn(|i| tree_mask[i] || building_mask[i]);
    ...
    pub fn build_mesh(
        ...
        let vertex_light = |voxel: [u32; 3],
         ...
         -> [u8; 2] {
            ...
            let q = |sum: u32| -> u8 {
                ((sum as f32 / n as f32) / 15.0 * 255.0)
                    .round()
            ...
            if !greedy {
                ...
                for unit_quad in group.iter() {
                    ...
                    for (i, p) in mesh_positions.iter().enumerate() {
                        ...
                        light[i] =
                            vertex_light(unit_quad.minimum, norm_ax, norm_sign, ax_u, ax_v, cu, cv);
                    }
                    emit_quad(
                        &mut positions,
                        &mut normals,
                ...
                for ((_slice, _v), mut row) in rows {
                    ...
                    while i < row.len() {
                        ...
                        while i + 1 < row.len()
                            && row[i + 1].0 == end_u + 1
                            && row[i + 1].1 == block
                            && end_u < 16  // never extend into padding (padded index 17)
        ...
        #[cfg(debug_assertions)]
        {
            ...
            if has_solid && vert_count == 0 {
                ...
                if has_exposed {
                    ...
                    for y in 0..CHUNK_SIZE {
                        for z in 0..CHUNK_SIZE {
                            // -X (padded x=0)
                            if padded[PaddedChunkShape::linearize([
                                0,
                                (y + 1) as u32,
                                (z + 1) as u32,
                            ]) as usize]
            ...
            for (i, uv) in uvs.iter().enumerate() {
                if !uv[0].is_finite() || !uv[1].is_finite() || uv[0] < 0.0 || uv[1] < 0.0 {
                    bevy::log::warn!(
...
mod tests {
    ...
    const NO_NEIGHBORS: ChunkNeighbors = ChunkNeighbors {
        neighbors: [None; 6],
    };
    ...
    fn buried_chunk_produces_no_geometry() {
        ...
        assert_eq!(
            verts, 0,
            "buried chunk emitted {} hidden-wall vertices",
            verts
        );
    }
    ...
    fn surface_chunk_mesh_is_well_formed() {
        ...
        for greedy in [false, true] {
            ...
            assert!(
                verts > 0,
                "surface chunk produced no geometry (greedy={greedy})"
            );
    ...
    fn greedy_never_exceeds_naive() {
        ...
        let naive = chunk
            .build_mesh(&NO_NEIGHBORS, false, false, 1.0, None)
            .count_vertices();
        let greedy = chunk
            .build_mesh(&NO_NEIGHBORS, true, false, 1.0, None)
            .count_vertices();
