...
/// True if no tree trunk stands within `radius` blocks of the given column.
///
/// Trees are stamped into chunks during generation, so they are invisible to
/// `ChunkManager::block_at` until the containing chunk has actually loaded.
/// Anything that needs to know about a tree *before* then — picking a spawn
/// point, for instance — has to ask the placement function directly, which is
/// what this does.
pub fn is_clear_of_trees(x: f32, z: f32, radius: f32) -> bool {
    let step = 18.0_f32;
    // Trees jitter by up to ±0.425 * step from their cell centre, so a trunk
    // can reach one cell further than a naive containment test would suggest.
    let reach = radius + step * 0.5;
    let i0 = ((x - reach) / step).floor() as i32;
    let i1 = ((x + reach) / step).ceil() as i32;
    let j0 = ((z - reach) / step).floor() as i32;
    let j1 = ((z + reach) / step).ceil() as i32;

    for i in i0..=i1 {
        for j in j0..=j1 {
            if let Some((tx, tz, _, scale)) = tree_at_grid(i, j) {
                // Wider trees need proportionally more room.
                let keep_out = radius * scale;
                if (x - tx).abs() < keep_out && (z - tz).abs() < keep_out {
                    return false;
                }
            }
        }
