...
use crate::chunk::{CHUNK_SIZE, Chunk, ChunkNeighbors, ChunkPos};
...
// `pos` is written but not read back: chunk lookups go through ChunkManager's
// maps, which are keyed by position already. It stays because it makes a chunk
// entity self-describing in a debugger or an entity inspector, where the map key
// is not to hand.
#[allow(dead_code)]
#[derive(Component)]
...
#[derive(Component)]
// `pos` duplicates the key the task was filed under; kept because it makes a
// task self-describing in a debugger, where the map key is not to hand.
#[allow(dead_code)]
struct ComputeChunk {
    pos: ChunkPos,
...
impl ChunkManager {
    ...
    pub fn clear_all(&mut self) {
        ...
        crate::chunk::GREEDY_INVARIANT_VIOLATED.store(false, std::sync::atomic::Ordering::Relaxed);
    }
    ...
    pub fn set_block(&mut self, world_pos: IVec3, block_type: BlockType) {
        ...
        let boundary_neighbor =
            |dirty: &mut HashSet<ChunkPos>, light: &mut HashSet<ChunkPos>, pos: ChunkPos| {
                dirty.insert(pos);
                light.insert(pos);
            };
        if local.x == 0 {
...
impl Plugin for ChunkManagerPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ChunkManager>()
            ...
            // SESSION RESET. ChunkManager is a resource, so it outlives the
            // world teardown that despawns its chunk entities. Without this it
            // enters the next session still believing every chunk is loaded —
            // its `chunks` map pointing at despawned entities — so load_chunks
            // never refills the bubble and the new world comes up almost empty.
            .add_systems(OnExit(crate::GameState::Gameplay), reset_for_new_world)
...
pub fn write_layer_solid(data: &mut [u8], layer: u8, color: Color, tile_size: u32, mip_count: u32) {
    ...
    for py in 0..tile_size {
        for px in 0..tile_size {
            let is_border =
                px < border || px >= tile_size - border || py < border || py >= tile_size - border;
    ...
    for m in 1..mip_count {
        ...
        for dy in 0..dst_size {
            for dx in 0..dst_size {
                ...
                for c in 0..3 {
                    layer_slice[di + c] = (linear_to_srgb(sum_rgb[c] * 0.25) * 255.0)
                        .round()
...
fn setup_chunk_material(
    ...
    #[cfg(debug_assertions)]
    {
        ...
        debug_assert!(
            img_valid,
            "Layer-array image handle is invalid after creation"
        );
    }
...
/// Build the full layer-major data blob for all block layers: solid colors and
...
fn build_layers_data(
    ...
    for &block in &crate::block_types::ALL_BUILTIN_BLOCKS {
        ...
        for tex_path in &try_paths {
            if let Ok(img_data) = std::fs::read(tex_path)
                && let Ok(dyn_img) = image::load_from_memory(&img_data)
            {
    ...
    for (slot, def) in settings.custom_blocks.iter().enumerate() {
            ...
            && let Ok(dyn_img) = image::load_from_memory(&img_data)
        {
            ...
            info!(
                "Loaded custom block texture: {} ({}x{})",
                def.name, tile_size, tile_size
            );
        }
    }
...
fn make_layer_array_image(data: Vec<u8>, tile_size: u32, mip_count: u32, aniso: u16) -> Image {
    ...
    use bevy::render::render_resource::{
        Extent3d, TextureDataOrder, TextureDimension, TextureFormat,
    };
...
fn apply_modifications(chunk: &mut Chunk, modifications: &HashMap<IVec3, BlockType>) -> [bool; 6] {
...
// Parameter count is how a Bevy system declares its data dependencies; splitting
// it up to satisfy the lint would obscure them rather than simplify anything.
#[allow(clippy::too_many_arguments)]
// `opt_flags` is only written by the debug-only greedy-meshing kill-switch
// below, so in a release build the binding is never mutated. The access still
// has to be `ResMut` for the debug path to compile.
    ...
    for (entity, mut compute) in &mut tasks {
        ...
        if let Some((mut chunk, tree_mask)) = check_ready(&mut compute.task) {
            ...
            if pos.0.abs() <= 1 && pos.1.abs() <= 1 && pos.2.abs() <= 1 {
                ...
                // Only flag a chunk that SHOULD contain ground. The 0.18 version
                // warned on any all-air chunk in the 3x3x3 neighbourhood of the
                // origin, but the surface near origin sits at y=5, so the whole
                // pos.1 == 1 layer (world y 16..31) is legitimately empty sky and
                // tripped this every run — with a message blaming save data.
                let center_surface = crate::terrain::surface_y(
                    pos.0 * CHUNK_SIZE + CHUNK_SIZE / 2,
                    pos.2 * CHUNK_SIZE + CHUNK_SIZE / 2,
                );
                if solid == 0 && pos.1 * CHUNK_SIZE <= center_surface {
                    bevy::log::warn!(
            ...
            // EMPTY-MESH CONTRACT (Bevy 0.19). Never create a mesh asset for a
            // chunk that meshed to zero vertices. Bevy 0.19's mesh allocator
            // skips slab allocation for a zero-vertex mesh
            // (`allocate_meshes`: `if vertex_buffer_size == 0 { continue }`) but
            // its copy loop has no matching guard, so `copy_element_data` runs
            // against a key that was never allocated and logs
            // "Use-after-free: attempted to copy element data for an
            // unallocated key" every extraction. A voxel world produces empty
            // meshes constantly — every all-air chunk above the surface and
            // every fully-occluded interior chunk — which buried the log in
            // ~100 errors/sec. The entity is still spawned and tracked; it
            // simply carries no geometry until a remesh gives it some.
            let vert_count = mesh.count_vertices();
            let is_empty = vert_count == 0;
            ...
            if !is_empty {
                ec.insert((
                    ...
                    MeshMaterial3d(chunk_mat.handle.clone()),
                ));
            }
...
fn process_light_queue(
    ...
    for pos in work {
        let result = {
            ...
            let new_light = crate::voxel_light::compute_chunk_light(&chunk.blocks, pos, &neighbors);
            if new_light == chunk.light {
                None
            } else {
                let faces = crate::voxel_light::boundary_faces_changed(&chunk.light, &new_light);
...
// The query type is inherently shaped like this — a type alias would just move
// the same structure somewhere the reader has to go look it up.
#[allow(clippy::type_complexity)]
fn remesh_dirty_chunks(
    mut commands: Commands,
    mut manager: ResMut<ChunkManager>,
    mut meshes: ResMut<Assets<Mesh>>,
    // Mesh3d is Option: per the EMPTY-MESH CONTRACT in handle_chunk_tasks, a
    // chunk that meshed to zero vertices carries no Mesh3d at all. Such a chunk
    // must still be remeshable — placing a block in an all-air chunk has to give
    // it geometry — so this query must match it rather than filter it out.
    mesh_query: Query<
        (
            Entity,
            Option<&Mesh3d>,
            Option<&MeshMaterial3d<BlockArrayMaterial>>,
        ),
        With<ChunkMarker>,
    >,
    chunk_material: Option<Res<ChunkMaterial>>,
    ...
    for pos in work {
        ...
        // atlas material.
        //
        // Four cases, because of the EMPTY-MESH CONTRACT (see handle_chunk_tasks):
        // a chunk with no geometry carries no Mesh3d, so both the component and
        // the new mesh may independently be present or absent.
        if let Ok((ent, maybe_mesh3d, maybe_mat)) = mesh_query.get(entity) {
            match (maybe_mesh3d, mesh.count_vertices() == 0) {
                // Had geometry, still has geometry: replace the asset in place.
                (Some(mesh3d), false) => {
                    let _ = meshes.insert(&mesh3d.0, mesh);
                }
                // Was empty, now has geometry (e.g. a block placed in an all-air
                // chunk): create the asset and attach the render components.
                (None, false) => {
                ...
                // Had geometry, now empty (last block dug out): drop the render
                // components rather than storing a zero-vertex mesh, which would
                // resume the allocator error spam.
                (Some(_), true) => {
                    commands
                        .entity(ent)
                        .remove::<Mesh3d>()
                        .remove::<MeshMaterial3d<BlockArrayMaterial>>();
                }
            ...
            if maybe_mesh3d.is_some()
                && maybe_mat.is_none()
                && let Some(ref mat) = chunk_material
            {
                commands
                    .entity(ent)
...
fn apply_texture_settings_change(
    ...
    if new_tile != atlas.tile_size {
        ...
        // Bevy 0.19: `Assets::get_mut` yields an `AssetMut<Image>` smart pointer
        // rather than `&mut Image`, so the binding itself must be `mut` to reach
        // `DerefMut`. Writing through it is what flags the asset as changed and
        // triggers the GPU re-upload, so `get_mut_untracked` is NOT the fix here.
        if let Some(mut image) = images.get_mut(&atlas.image_handle) {
        ...
        info!(
            "Block layers rebuilt: {}px tiles, {} mips",
            new_tile, mip_count
        );
...
/// Drops all per-session chunk state on the way out of Gameplay.
///
/// See the SESSION RESET note in `ChunkManagerPlugin`: the entities are gone by
/// the time this runs, but the bookkeeping that referenced them is not.
pub(crate) fn reset_for_new_world(mut manager: ResMut<ChunkManager>) {
    manager.clear_all();
}
